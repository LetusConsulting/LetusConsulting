...
const ConsultantCard: React.FC<Props> = ({ consultant }) => {
  return (
    <div className="group relative rounded-lg border border-white/10 bg-card-dark p-6 md:p-8 hover:border-accent/40 transition-all duration-500">
      <div className="flex flex-col md:flex-row gap-8 items-start">
        {/* Avatar Section */}
        <div className="flex-shrink-0 flex flex-col items-center gap-4 mx-auto md:mx-0">
          <div className="relative h-40 w-40 rounded-full border-2 border-[#333] p-1 group-hover:border-accent transition-colors duration-500">
            <img
          ...
          {consultant.verified && (
            <div className="flex items-center gap-1 text-xs font-mono text-gray-500 group-hover:text-accent/80 transition-colors">
          ...
          {consultant.skills && consultant.skills.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8">
              {consultant.skills.map((skill, i) => (
                <div
                  key={i}
                  className="flex h-8 items-center justify-center gap-x-2 rounded-full border border-white/10 bg-transparent hover:bg-white/5 hover:border-accent/50 pl-4 pr-4 transition-colors cursor-default group/chip"
                >
                  <span className="material-symbols-outlined text-[16px] text-gray-500 group-hover/chip:text-accent transition-colors">
                    {skill.icon}
                  </span>
                ...
                {consultant.projects.map((project, idx) => (
                  <React.Fragment key={idx}>
                    <div className="flex flex-col items-center">
                      <div className="h-2 w-2 rounded-full bg-accent mt-2 shadow-[0_0_8px_rgba(56,189,248,0.5)]"></div>
